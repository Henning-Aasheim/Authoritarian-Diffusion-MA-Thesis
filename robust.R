...
base <- base %>% 
  mutate(delta_fbic_1 = fbic - lag(fbic, n = 1),
         ...
         delta_fbic_3 = fbic - lag(fbic, n = 3),
...
robust_m6_delta_3 <- feols(l(freedom, 1) ~ delta_fbic_3 + gdppc_log + rents + oda + west_2_fbic + factor(regime) |
                             country + year, 
                           data     = base,
                           cluster  = 'country', 
                           panel.id = ~country+year)
...
robust_map_delta <- list(
  ...
  'delta_fbic_3'    = 'Linkages to China',
...
robust_m5_delta_3 <- feols(l(freedom, 1) ~ delta_fbic_3*factor(regime) + gdppc_log + rents + oda + west_2_fbic | 
                             country+ year, 
                           data     = base, 
                           cluster  = 'country', 
                           panel.id = ~country+year)
...
robust_interaction_delta_map <- list(
  ...
  'delta_fbic_3:factor(regime)1' = 'China x El.Aut.',
  ...
  'delta_fbic_3:factor(regime)2' = 'China x El.Dem.',
  ...
  'delta_fbic_3:factor(regime)3' = 'China x Lib.Dem.',
